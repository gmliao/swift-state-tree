...
import { decodeMessage, encodeMessage, type TransportEncoding } from './protocol'
import { NoOpLogger, type Logger } from './logger'
import { StateTreeView, type ViewOptions } from './view'

export interface StateTreeRuntimeOptions {
  logger?: Logger
...
export class StateTreeRuntime {
  ...
  private transportEncoding: TransportEncoding
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        this.ws = await createWebSocket(url, {
          binaryType: this.transportEncoding === 'messagepack' ? 'arraybuffer' : undefined
        })
  ...
  sendRawMessage(message: TransportMessage): void {
    ...
    const encoded = encodeMessage(message, this.transportEncoding)
  ...
  /**
   * Current transport encoding for WebSocket payloads.
   */
  get encoding(): TransportEncoding {
    return this.transportEncoding
  }
  ...
  private async handleMessage(data: unknown): Promise<void> {
    try {
      ...
      if (this.transportEncoding === 'messagepack') {
        if (typeof data === 'string') {
          this.logger.warn('Received text frame in MessagePack mode, attempting JSON decode')
          decoded = decodeMessage(data, 'json')
        } else {
          ...
          if (data instanceof ArrayBuffer) {
            ...
            this.logger.warn(`Unexpected data type: ${typeName}, attempting to convert to bytes`)
            bytes = typeof TextEncoder !== 'undefined'
              ? new TextEncoder().encode(String(data))
              : new Uint8Array()
          }
          decoded = decodeMessage(bytes, 'messagepack')
        }
      } else {
        let text: string
        ...
        decoded = decodeMessage(text, 'json')
      }
