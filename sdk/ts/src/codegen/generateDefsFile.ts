...
function generateDeterministicMathHelpers(schema: ProtocolSchema): string {
  ...
  lines.push('')
  lines.push('/**')
  lines.push(' * Round a number using the same rule as Swift\'s .toNearestOrAwayFromZero')
  lines.push(' * This ensures cross-platform consistency with Swift FixedPoint.quantize()')
  lines.push(' * For negative half values (e.g., -1.5), rounds away from zero (e.g., -2)')
  lines.push(' * For positive half values (e.g., 1.5), rounds away from zero (e.g., 2)')
  lines.push(' * @param value - The number to round')
  lines.push(' * @returns The rounded integer')
  lines.push(' */')
  lines.push('function roundToNearestOrAwayFromZero(value: number): number {')
  lines.push('  if (value >= 0) {')
  lines.push('    return Math.floor(value + 0.5)')
  lines.push('  } else {')
  lines.push('    return Math.ceil(value - 0.5)')
  lines.push('  }')
  lines.push('}')
  lines.push('')
  ...
  if (hasIVec2) {
    ...
    lines.push(`  return { x: roundToNearestOrAwayFromZero(x * ${SCALE}), y: roundToNearestOrAwayFromZero(y * ${SCALE}) }`)
    lines.push('}')
    lines.push('')
  }
  ...
  if (hasIVec3) {
    ...
    lines.push(`  return { x: roundToNearestOrAwayFromZero(x * ${SCALE}), y: roundToNearestOrAwayFromZero(y * ${SCALE}), z: roundToNearestOrAwayFromZero(z * ${SCALE}) }`)
    lines.push('}')
    lines.push('')
  }
