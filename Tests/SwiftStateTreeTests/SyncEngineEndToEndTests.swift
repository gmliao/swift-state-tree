    ...
    func testE2E_Create_AddPlayer() throws {
        ...
        for (key, value) in jsonObject {
            ...
            if let number = value as? NSNumber {
                // Check if it's a boolean, integer, or double
                // In JSON, booleans are represented as NSNumber with objCType "c" (char)
                // We can check by comparing the string representation or objCType
