...
export class SwiftStateTreeClient {
  ...
  private handleMessage(data: WebSocket.Data) {
    try {
      ...
      if (Array.isArray(json) && json.length >= 1) {
        ...
        const isTransportMessageOpcode =
        ...
        if (isTransportMessageOpcode) {
          ...
          console.log(
            chalk.cyan(
              `📦 TransportMessage [${messageType}] (opcode: ${firstElement}):`,
            ),
          );
          console.log(
            chalk.gray(
              `   Raw array (first 500 chars): ${JSON.stringify(json).substring(0, 500)}...`,
            ),
          );
          ...
          if (firstElement === 105 && json.length >= 3) {
            ...
            console.log(
              chalk.cyan(
                `   JoinResponse: requestID=${requestID}, success=${success}`,
              ),
            );
            if (landType) console.log(chalk.gray(`   landType: ${landType}`));
            if (landInstanceId)
              console.log(chalk.gray(`   landInstanceId: ${landInstanceId}`));
            if (playerSlot != null)
              console.log(chalk.gray(`   playerSlot: ${playerSlot}`));
          ...
          if (firstElement === 103 && json.length >= 4) {
            ...
            console.log(
              chalk.cyan(`   Event: ${directionName}, type: ${typeStr}`),
            );
            if (Array.isArray(payload)) {
              console.log(
                chalk.gray(`   Payload array (length: ${payload.length}):`),
              );
              payload.forEach((item: any, index: number) => {
                ...
                console.log(
                  chalk.gray(
                    `     [${index}]: ${itemStr}${typeof item === "object" ? "..." : ""}`,
                  ),
                );
              });
            } else {
      ...
      if (Array.isArray(json) && json.length >= 2) {
        ...
        if (isOpcodeFormat) {
          ...
          console.log(
            chalk.cyan(
              `📦 Opcode StateUpdate [${opcodeName}]: ${playerIDType}=${playerIDValue}, patches=${patchCount}`,
            ),
          );
          console.log(
            chalk.gray(
              `   Raw payload (first 200 chars): ${JSON.stringify(json).substring(0, 200)}...`,
            ),
          );
          ...
          if (patchCount > 0 && json.length > 2) {
            ...
            for (let i = 0; i < previewCount; i++) {
              ...
              const patchStr = Array.isArray(patch)
                ? JSON.stringify(patch).substring(0, 100)
                : String(patch).substring(0, 100);
      ...
      if (json && typeof json === "object" && "payload" in json) {
        ...
        if (
          ...
          if (errorPayload.details) {
            console.log(
              chalk.gray(`   Details: ${JSON.stringify(errorPayload.details)}`),
            );
          }
          return;
        }
    ...
    switch (message.kind) {
      case "joinResponse": {
        ...
        if (success) {
          ...
          if (payload.landID && payload.landID !== this.landID) {
            console.log(
              chalk.yellow(
                `ℹ️  LandID updated: ${this.landID} -> ${payload.landID}`,
              ),
            );
            this.landID = payload.landID;
          }

          const encodingInfo = payload.encoding
            ? chalk.yellow(`, encoding=${payload.encoding}`)
            : "";
          console.log(
            chalk.green(
              `✅ Join successful: playerID=${payload.playerID || "unknown"}, landID=${this.landID}${encodingInfo}`,
            ),
          );
        } else {
          this.isJoined = false;
          console.log(
            chalk.red(`❌ Join failed: ${payload.reason || "unknown reason"}`),
          );
        }
      ...
      case "actionResponse": {
        ...
        if (callback) {
          ...
          console.log(
            chalk.cyan(
              `📥 Action response [${payload.requestID}]: ${JSON.stringify(payload.response)}`,
            ),
          );
        }
        break;
      ...
      case "event": {
        ...
        if (payload.fromServer) {
          ...
          console.log(
            chalk.magenta(
              `📨 Server event [${eventData.type}]: ${JSON.stringify(eventData.payload)}`,
            ),
          );
          ...
          if (Array.isArray(eventData.payload)) {
            console.log(
              chalk.gray(
                `   Raw array payload (length: ${eventData.payload.length}):`,
              ),
            );
          ...
          console.log(
            chalk.blue(
              `📤 Client event echo [${eventData.type}]: ${JSON.stringify(eventData.payload)}`,
            ),
          );
        }
        break;
      ...
      case "error": {
        ...
        if (payload && "code" in payload && "message" in payload) {
          ...
          console.log(
            chalk.yellow(
              `⚠️  Error payload format: ${JSON.stringify(payload, null, 2)}`,
            ),
          );
          errorPayload = {
            code: "UNKNOWN_ERROR",
      ...
      default:
        console.log(
          chalk.gray(
            `📨 Message [${message.kind}]: ${JSON.stringify(message.payload).substring(0, 100)}`,
          ),
        );
    }
  }
  ...
  private handleStateUpdate(update: StateUpdate) {
    ...
    console.log(
      chalk.yellow(`🔄 State update [${update.type}]: ${patchCount} patches`),
    );
  ...
  private handleSnapshot(snapshot: StateSnapshot) {
    ...
    console.log(
      chalk.gray(
        `   State: ${JSON.stringify(this.currentState, null, 2).substring(0, 200)}...`,
      ),
    );
  }
  ...
  private applyPatch(patch: StatePatch) {
    ...
    if (parts.length === 1) {
      ...
      // Nested property
      if (
        !(key in this.currentState) ||
        typeof this.currentState[key] !== "object" ||
        this.currentState[key] === null
      ) {
        this.currentState[key] = {};
      }
      this.applyNestedPatch(this.currentState[key], {
        ...patch,
        path: restPath,
      });
    }
  ...
  async join(): Promise<{
    ...
    return new Promise((resolve) => {
      ...
      const landType =
        colonIndex > 0 ? this.landID.substring(0, colonIndex) : this.landID;
      const landInstanceId =
        colonIndex > 0 ? this.landID.substring(colonIndex + 1) : null;
      ...
      console.log(
        chalk.blue(
          `📤 Join request sent: landType=${landType}, landInstanceId=${landInstanceId ?? "null"}`,
        ),
      );
    });
  }
  ...
  async sendAction(actionType: string, payload: any): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(
        chalk.blue(
          `📤 Action sent [${actionType}]: ${JSON.stringify(payload)}`,
        ),
      );
    });
  }
  ...
  sendEvent(eventType: string, payload: any) {
    ...
    this.send(message as any);
    console.log(
      chalk.blue(`📤 Event sent [${eventType}]: ${JSON.stringify(payload)}`),
    );
  }
