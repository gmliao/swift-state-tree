...
import { StateTreeRuntime } from '../../../../sdk/ts/src/core/runtime'
import { StateTreeView } from '../../../../sdk/ts/src/core/view'
import { createWebSocket } from '../../../../sdk/ts/src/core/websocket'
import type { TransportMessage } from '../../../../sdk/ts/src/types/transport'

// Mock WebSocket for testing
class MockWebSocket {
  private onMessageCallback?: (event: { data: string | ArrayBuffer }) => void
  ...
  simulateMessage(data: string | ArrayBuffer) {
    // Use setTimeout to simulate async message handling
    setTimeout(() => {
  ...
  // Add properties that runtime might check
  readyState = 1 // OPEN
  CONNECTING = 0
  OPEN = 1
  CLOSING = 2
...
async function testEncodingPropagation() {
  ...
  // Create a simple view with proper schema structure
  const view = (runtime as any).createView('test-land', {
    schema: {
      schemaHash: 'test',
      lands: {
        'test-land': {
          stateType: 'TestLandState',
          actions: {},
          clientEvents: {},
          serverEvents: {}
        }
  ...
  // Start join but don't await yet
  const joinPromise = view.join()
  
  // Wait a bit for the message to be sent and onmessage to be set
  ...
  // Directly call runtime's handleMessage to simulate receiving the message
  // This bypasses WebSocket event system which may not work correctly with mock
  const handleMessage = (runtime as any).handleMessage.bind(runtime)
  await handleMessage(joinResponseEncoded)
  
  // Wait a bit for view to process the message
  await new Promise(resolve => setTimeout(resolve, 50))
  
  // Now await the join to complete (with timeout)
  try {
    await Promise.race([
      joinPromise,
      new Promise((_, reject) => setTimeout(() => reject(new Error('Join timeout')), 1000))
    ])
  } catch (e) {
    // If join times out, that's okay for this test - we just need to check encoding
    console.log('   ⚠️  Join timeout (expected in mock environment), checking encoding...')
  }
