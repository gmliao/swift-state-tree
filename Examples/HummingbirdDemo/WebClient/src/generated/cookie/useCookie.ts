...
export function useCookie(): CookieComposableReturn {
  ...
  async function buyUpgrade(payload: BuyUpgradeAction): Promise<BuyUpgradeResponse> {
    if (!tree.value || !isJoined.value) {
      throw new Error('Not connected or not joined')
    }
    try {
      const res = await tree.value.actions.buyUpgrade(payload)
      lastError.value = null
      return res
    } catch (error) {
  ...
  async function clickCookie(payload: ClickCookieEvent): Promise<void> {
    if (!tree.value || !isJoined.value) return
    try {
      await tree.value.events.clickCookie(payload)
      lastError.value = null
    } catch (error) {
