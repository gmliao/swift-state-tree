...
export class HeroDefenseStateTree {
  ...
  constructor(runtime: StateTreeRuntime, options?: StateTreeOptions) {
    ...
    // Initialize players map subscriptions
    this.players = {
      onAdd: (callback) => {
        return this.view.onPatch((patch, decodedValue) => {
          if (patch.op === 'add') {
            const match = patch.path.match(/^\/players\/([^/]+)$/)
            if (match) {
              callback(match[1], decodedValue as PlayerState)
            }
          }
        })
      },
      onRemove: (callback) => {
        return this.view.onPatch((patch) => {
          if (patch.op === 'remove') {
            const match = patch.path.match(/^\/players\/([^/]+)$/)
            if (match) {
              callback(match[1])
