...
export class PlayerManager {
  ...
  private onCurrentPlayerUpdate:
    | ((player: PlayerSprite, serverPos: { x: number; y: number }) => void)
    | null = null;
  ...
  subscribeToTree(
    ...
    this.unsubscribeAdd = tree.players.onAdd(
      (playerID: string, playerState: PlayerState) => {
  ...
  private logStateUpdate(_playersState: PlayersState): void {
    const now = Date.now();
    if (
      !this.lastStateLogTime ||
      now - this.lastStateLogTime > this.config.logThrottleMs
    ) {
