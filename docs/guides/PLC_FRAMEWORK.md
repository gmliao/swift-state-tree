...
| 屬性 | 對成本的典型影響 |
|------|-----------------|
| **Placement (位置)** | 放得越遠 → Movement Cost ↑ |
| | 放得越近 (memory) → Movement Cost ↓ |
| **Lifetime (生命週期)** | 活越久 → 維護更新的總成本 ↑ |
| | 活越短 → 允許更多暫時不一致,負擔 ↓ |
| **Consistency (一致性)** | 要求越強 → Compute Cost ↑ |
| | 同時 Movement Cost ↑ (更多同步) |

實際上三個屬性會「一起作用」:
- 高 Lifetime + 遠 Placement + 強 Consistency → 最貴的一種狀態 (例如:金融交易主帳本)
- 短 Lifetime + 近 Placement + 弱 Consistency → 最便宜的一種狀態 (例如:暫時 UI 狀態,快取)
...
這套 PLC + Cost + Capacity + 系統分層的模型,
把原本全靠直覺的「架構決策」,
變成可以表格化,可討論,可預測的一套「狀態工程學」.

以後你要分析任何系統 (遊戲,Web,即時協作,登入系統...):
1. 把重要的 state 列出來
2. 為每個 state 填 PLC
3. 估計它們的 Cost (大致高低也可以)
4. 對照現在系統的 Capacity
5. 找出哪裡不合理,哪裡要調整,哪裡該重寫,哪裡該搬家
