...
在分散式領域裡已有許多框架與理論，例如 CAP/PACELC、CQRS、CRDT 等，專注於整體架構層級的取捨。
PLC 並不是新的分散式理論，而是將 CAP/PACELC 的架構策略轉化為每筆狀態的具體設計決策。

CQRS 與 CRDT 分別處理操作模式與資料合併問題，而 PLC 專注在「每個狀態本身該如何放置與維護」。它們互補但不重疊。

簡單來說：**CAP/PACELC 是「整體系統」的生存策略；PLC 是「每一份狀態」的最佳決策指南。**
...
- 調整 Consistency (強一致 → session-level → 弱一致) → 降低不必要的更新頻率
- 合併多個變更成一個
- 做 Diff,做快取
