...
///   - Delegates to `StateTreeProtocol.snapshot(for:)` which is generated at compile time by the `@StateTreeBuilder` macro
...
public struct SyncEngine: Sendable {
    ...
    /// Generate a full snapshot of the StateTree for a specific player or broadcast fields only
    ///
    /// This method is used for late join scenarios where a player needs to receive the complete
    /// state snapshot. It delegates to the `snapshot(for:)` method generated by the `@StateTreeBuilder` macro,
    ...
    public func snapshot<State: StateTreeProtocol>(
        ...
        let newSnapshot = try state.snapshot(for: playerID)
    ...
    public func extractBroadcastSnapshotMirrorVersion<State: StateTreeProtocol>(
        ...
        // Use reflection to access broadcast field values directly from StateTree
    ...
    private func extractPerPlayerSnapshot<State: StateTreeProtocol>(
        for playerID: PlayerID,
        from state: State
    ) throws -> StateSnapshot {
        // Generate full snapshot for the specific player
        let fullSnapshot = try state.snapshot(for: playerID)
    ...
    private mutating func computeBroadcastDiff<State: StateTreeProtocol>(
        from state: State,
    ...
    private mutating func computePerPlayerDiff<State: StateTreeProtocol>(
        for playerID: PlayerID,
        from state: State,
        onlyPaths: Set<String>?
    ) throws -> [StatePatch] {
    ...
    ///   - state: The current StateTree instance
    ...
    private func isBroadcastPath<State: StateTreeProtocol>(_ path: String, state: State) -> Bool {
    ...
    private func isPerPlayerPath<State: StateTreeProtocol>(_ path: String, state: State) -> Bool {
        // Extract field name from path (first segment after "/")
        let pathComponents = path.split(separator: "/", omittingEmptySubsequences: true)
        guard let fieldName = pathComponents.first.map(String.init) else {
            return false
    ...
    ///   - state: The StateTree instance
    /// - Throws: `SyncError` if value conversion fails
    private mutating func populateCacheIfNeeded<State: StateTreeProtocol>(
        for playerID: PlayerID,
        from state: State
    ) throws {
    ...
    /// - Throws: `SyncError` if value conversion fails
    public mutating func lateJoinSnapshot<State: StateTreeProtocol>(
        for playerID: PlayerID,
        from state: State
    ) throws -> StateSnapshot {
        // Generate snapshot
