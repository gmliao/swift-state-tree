...
/// - `extractBroadcastFields()` method implementation (for efficient broadcast field extraction)
///
/// Example:
/// ```swift
/// @StateTreeBuilder
...
public protocol StateTreeProtocol: Sendable {
    ...
    /// Generate a snapshot of the StateTree for a specific player or broadcast fields only
    /// 
    ...
    /// Extract broadcast fields only (for efficient broadcast snapshot generation)
    ///
    /// Returns a `StateSnapshot` containing only broadcast fields, without needing a player ID.
    /// This method is optimized for broadcast field extraction and avoids runtime reflection.
    ///
    /// This method is automatically generated by the `@StateTreeBuilder` macro.
    ...
    /// Example:
    /// ```swift
    /// let broadcastSnapshot = gameState.extractBroadcastFields()
    /// // Returns: StateSnapshot with only broadcast fields (players, round, turn, etc.)
    /// ```
    func extractBroadcastFields() throws -> StateSnapshot
}
