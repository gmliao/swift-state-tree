...
struct MessagePackDirectDecoder {
    func decodeTransportMessage(from value: MessagePackValue) throws -> TransportMessage {
        ...
        case .event:
            ...
            return TransportMessage(kind: kind, payload: .event(try decodeTransportEventPayload(from: eventValue)))
        case .join:
    ...
    func decodeActionEnvelope(from value: MessagePackValue) throws -> ActionEnvelope {
        ...
        let payloadData = try decodeBinaryData(from: payloadValue, context: "action payload")
        return ActionEnvelope(typeIdentifier: typeIdentifier, payload: payloadData)
    }
    ...
    private func decodeTransportActionPayload(from value: MessagePackValue) throws -> TransportActionPayload {
        ...
        let landID = try decodeRequiredString(map["landID"], context: "landID")
        ...
        return TransportActionPayload(requestID: requestID, landID: landID, action: action)
    }
    ...
    private func decodeTransportEventPayload(from value: MessagePackValue) throws -> TransportEventPayload {
        let map = try stringKeyedMap(from: value, context: "event payload")
        let landID = try decodeRequiredString(map["landID"], context: "landID")
        guard let eventValue = map["event"] else {
            throw MessagePackDirectDecodingError.invalidFormat("Missing event")
        }
        let event = try decodeTransportEvent(from: eventValue)
        return TransportEventPayload(landID: landID, event: event)
    }
